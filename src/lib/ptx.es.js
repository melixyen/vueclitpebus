...
"function"===typeof define&&define.amd?define(function(){return r}):"undefined"!==typeof exports?("undefined"!==typeof module&&module.exports&&(module.exports=r),exports=r):G.jsSHA=r;})(me);
//jsSHA function end
    ...
    constructor(companyTag) {
        ...
        let methodObj = {
            ...
            getFromToTravelTime: function(fromID, toID, cfg = {}){
                ...
                return me.getS2STravelTime(LineID, cfg).then(function(res){
                    ...
                    if(!aryTravelTimes) return {status:CM.CONST_PTX_API_FAIL, error: 'No match any route.'};
        ...
        let catchData = {
            ...
            calcLineTimeByFirstStation: function(LineObj, FirstStationID, FirstStationTime, RouteID, Direction){
                ...
                return time.map(c=>CM.transSec2Time(c))
            },
...
const companyTag$1 = metro.getCompanyTag('trtc');
...
var fnMRT = {
    ...
    getStationTime: function(LineID, StationID, w, cbFn){
        ...
        CM.pui.printStatus('線上尋找捷運 ' + StationID + ' 站時刻表');
        //產生暫存時刻表空間
...
const companyTag$2 = metro.getCompanyTag('krtc');
...
const companyTag$3 = metro.getCompanyTag('tymetro');
...
mrtPTXFn$2.catchData.config.Station_Fare_BackTag = ['OriginStationID','DestinationStationID','Fares','TrainType'];
//Catch Data 資料預處理
...
const companyTag$4 = metro.getCompanyTag('klrt');
...
const traURL$1 = CM.traURL;
const traV3URL = CM.traV3URL;
const urls$1 = {
    Network: traURL$1 + '/Network', //取得臺鐵路網資料
...
let getPTX$1 = ptx.getPromiseURL;

function setDefaultCfg$1(cfg={}){
    if(typeof(cfg)=='string') cfg = {paramDirectlyUse: cfg};//若傳入的為字串代表直接用於最後的參數不需再調整
    cfg.cbFn = cfg.cbFn || function(data,e){};
...
function processCfg$1(cfg){//將 cfg 轉為對應的參數
    if(cfg.paramDirectlyUse) return cfg.paramDirectlyUse;
    var aryParam = [];
    if(cfg.selectField) aryParam.push(ptx.selectFieldFn(cfg.selectField));
    if(cfg.filterBy) aryParam.push(ptx.filterFn(cfg.filterBy));
...
var tra = {
    ...
    getStationTodayTimeTable: function(StationID, cfg={}){
        let date = new Date();
        let dateStr = date.getFullYear() + '-' + CM.appendNumber0(date.getMonth()+1) + '-' + CM.appendNumber0(date.getDate());
...
let catchData$1 = {
    ...
    SimpleTimetable: function(progressFn){
        return catchData$1.GeneralTimetable(progressFn)
        .then(function(json){
...
aryMakeFunction$1.forEach(function(fn){
    if(!/\{/.test(urls$1[fn])){//排除要傳參數組 URL 的
        tra['_' + fn] = function(cfg){return makePTX_func$1(fn, cfg);};
        ptxAutoTRAFunctionKey.push('_' + fn);
    }else{//處理有動態參數的
        let urlAry = urls$1[fn].split('/');
        let paramCount = 0;
        let paramAry = [];
